...
export default function Modal({ show, modalTitle, fields, handleSubmit, handleModal, dropdownFields }) {
    if (!show) {
        return null;
    }
        ...
        <div className="absolute inset-0 bg-gray-400 bg-opacity-60 flex justify-center items-center">
            <div className="bg-gray-200 z-50 opacity-100 w-full max-w-lg md:w-1/2 lg:w-1/3 p-5 rounded-lg shadow-lg flex flex-col items-center">
                ...
                <div className="w-full flex justify-between items-center p-3 rounded-t-lg">
                    <div />
                    <h2 className="text-lg translate-x-3 font-semibold">{modalTitle}</h2>
                    <Button
                ...
                {/* Formulario */}
                <Form buttonText="Guardar" formFields={fields} dropdownFields={dropdownFields} handleSubmit={handleSubmit} />
            </div>
        </div>
